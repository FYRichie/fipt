...
def get_direction(k, img_hw):
    ...
    """
    screen_y, screen_x = torch.meshgrid(torch.linspace(0.5, img_hw[0] - 0.5, img_hw[0]), torch.linspace(0.5, img_hw[1] - 0.5, img_hw[1]))
    rays_d = torch.stack([(screen_x - k[0, 2]) / k[0, 0], (screen_y - k[1, 2]) / k[1, 1], torch.ones_like(screen_y)], -1).reshape(-1, 3)
    return rays_d
...
class RealDataset(Dataset):
    ...
    def __init__(self, root_dir, split="train", pixel=True, ray_diff=False, use_albedo=True):
        ...
        metadata = json.load(open(os.path.join(self.root_dir, "metadata.json")))
        C2Ws = [np.array(frame["camtoworld"])[:3, :4] for frame in metadata["frames"]]
        Ks = [np.array(frame["intrinsics"])[:3, :3] for frame in metadata["frames"]]
        ...
        if self.pixel:
            ...
            for idx in range(len(self.Ks)):
                ...
                if self.ray_diff:
                    ...
                    self.all_rays.append(torch.cat([rays_x, rays_d, dxdu, dydv, img], -1))
                else:
