...
class ModelTrainer(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.hparams.has_part:
            ...
            mean_metallic = torch_scatter.scatter(metallic.squeeze(-1) * weight_seg_, inv_idxs, 0, mean_metallic, reduce="sum").unsqueeze(-1)
            mean_roughness = torch_scatter.scatter(roughness.squeeze(-1) * weight_seg_, inv_idxs, 0, mean_roughness, reduce="sum").unsqueeze(-1)
            ...
            # propagation loss
            loss_seg = (metallic - mean_metallic[inv_idxs]).abs().mean() + (roughness - mean_roughness[inv_idxs]).abs().mean()
            ...
            weight_seg_ = torch.exp(-((albedo.data[ii] - albedo.data[jj]).pow(2).sum(-1) / self.hparams.sigma_albedo**2) / 2.0)
            weight_seg_ *= torch.exp(-((positions[ii] - positions[jj]).pow(2).sum(-1) / self.hparams.sigma_pos**2) / 2.0)
