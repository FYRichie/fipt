...
if __name__ == "__main__":
    ...
    for idx in tqdm(range(len(dataset))):
        ...
        inds = position[..., 0] + position[..., 1] * res_spatial + position[..., 2] * res_spatial * res_spatial
    ...
    torch.save({"mask": (SpatialHist > 0), "voxel_min": voxel_min.item(), "voxel_max": voxel_max.item(), "weight": vslf.state_dict()}, os.path.join(OUTPUT_PATH, "vslf.npz"))
    ...
    for batch in tqdm(dataset):
        ...
        for b in range(math.ceil(B * 1.0 / batch_size)):
            ...
            (
                ...
                _,
                _,
            ) = material_net.sample_diffuse(torch.rand((b1 - b0) * SPP, 2, device=device), normal[b0:b1].repeat_interleave(SPP, 0))

            p_next, _, _, _, valid_next = ray_intersect(scene, position[b0:b1].repeat_interleave(SPP, 0) + mitsuba.math.RayEpsilon * wi, wi.reshape(-1, 3))  # prevent self intersection
    ...
    for batch in tqdm(dataset):
        ...
        for r_idx, roughness in enumerate(roughness_level):
            ...
            for b in range(math.ceil(B * 1.0 / batch_size)):
                ...
                # importance sampling wi
                wi, _, g0, g1 = material_net.sample_specular(torch.rand((b1 - b0) * SPP, 2, device=device), wo[b0:b1].repeat_interleave(SPP, 0), normal[b0:b1].repeat_interleave(SPP, 0), roughness)
